...
- **Posted-Invoice UI Lock:** Sales (`SalesDocumentForm.tsx`, scoped to `isInvoice`) and purchase (`PurchaseInvoiceForm.tsx`) edit screens detect `existing.status === "posted"` and (a) render an amber Lock banner, (b) wrap the `<Tabs>` body in `<fieldset disabled className="contents">` so all inputs/Radix triggers become read-only without changing layout, (c) hide the Save button (Cancel→Back). Print button stays enabled. Backend already returns 409 on PUT/DELETE; this is pure UX hardening — admins can still unpost via existing PATCH `/sales-invoices/:id/unpost` and `/purchase-invoices/:id/unpost` from the list screen.

## Standalone "Add LC Expense" Screen
Route `/purchasing/lc-expense-entry` (permission: `purchase_invoices`). Lets users record LC expenses incrementally over time (shipping → customs → insurance → bank fees) without reopening the full LC edit dialog. Pick an open LC (closed LCs filtered out + blocked server-side), see live LC summary card (total / remaining / current expenses / loaded cost with FX awareness), then add a single expense (type / account / date / currency with auto-rate fetch / amount / notes). Optional "Pay Now" toggle posts the same balanced JE pattern the LC dialog uses (Dr expense account / Cr bank or cash) tagged `[LC#id][LCE#id]` so the LC Statement report links it. Backend: `POST /api/purchasing/letters-of-credit/:id/expenses` (returns 409 on closed LC). Recent expenses table for the selected LC plus link to the LC Statement filtered by lcId. Linked from purchasing sidebar + PurchasingHub tile (amber).
**Defaults fix (2026-05):** When picking a USD LC the form now defaults the new-expense `currencyCode` to the company **base currency (SAR) with rate=1**, NOT to the LC's foreign currency. Auto-flipping to USD/3.756 caused users to type a SAR amount under a foreign rate and inflated `amountBase = amount × rate`. Server-side guard added to all three insert sites (`POST /lc/:id/expenses`, `POST /lc`, `PUT /lc/:id`): if `currencyCode === baseCurrency`, `exchangeRate` is forced to `"1"` regardless of payload.

## Cost Centers — Activity & AI Analysis Tab
`/accounting/cost-centers` now has two tabs: **إدارة المراكز** (existing tree/CRUD view) and **الحركة والتحليل**. The analysis tab lets the user pick any cost center + date range and shows: 3 KPI cards (debit/credit/balance with green/red/blue gradients), a *grouped-by-account* aggregation table, and the full per-line transactions table with green/red badges per debit/credit + a deep-link to each journal entry. Backend joins `journal_entry_lines.cost_center` (text col holding the CC code) ↔ `journal_entries` ↔ `accounts` via `GET /api/cost-centers/:id/transactions?from=&to=`. AI insights: `POST /api/cost-centers/:id/ai-insights` mirrors the payment-mix-report pattern (gpt-5.4 via Replit AI proxy, JSON response `{headline, highlights, concerns, recommendation}`) and is rendered as a structured violet/fuchsia insights card. Permission-gated by the `accounts` module (same as the existing CRUD).
