...
- **Core Modules:**
    ...
    - **Financial Transactions (المعاملات المالية):** Standalone full-page UX for cash deposits / withdrawals / transfers between cash boxes and bank accounts (`/cash/financial-transactions`). Built on top of the existing `/api/cash-transfers` backend; maps the friendly user kind (deposit/withdraw/transfer) to the underlying `transferType` (cash_to_bank / bank_to_cash / cash_to_cash / bank_to_bank). Three-tab form: transaction details (segmented kind picker), parties & amount (with live journal-entry preview), and a chart-of-accounts tree tab for fast picking — clicking a tree account that is linked to a cash box / bank fills the compatible side automatically. Coexists with the legacy `/cash/transfers` page.
    - **Receipt Vouchers (سندات القبض):** Customer-only single-tab form (`/cash/receipt-vouchers/new`) — entity is always the customer (credit side); cash box / bank account is always the debit side. An optional toggle links the voucher to one of the customer's open sales invoices (`receipt_vouchers.salesInvoiceId`); when linked, the sales-invoices listing surfaces a "حالة السداد" (Payment Status) column showing `سُدِّد نقداً / سُدِّد بنكاً (RV-####)`. General account overrides (cash, bank, receivable defaults) live in the Accounting Mappings control panel under the `customer_settlement` group ("تسوية العملاء (سندات القبض)"); the per-voucher counter-account override and AI-suggest were removed. The backend defensively forces `entityType="customer"` and falls back to the mapping accounts when the cash/bank/customer record itself has no `accountId`.
    - **Payment Vouchers (سندات الصرف):** Mirrors the Receipt Voucher pattern with reversed JE direction. Supplier-only single-tab full-page form (`/cash/payment-vouchers/new` and `/:id`) — entity is always the supplier (debit side); cash box / bank account is always the credit side. An optional toggle links the voucher to one of the supplier's posted purchase invoices (`payment_vouchers.purchaseInvoiceId`, `ON DELETE SET NULL`); when linked, the purchase-invoices listing surfaces a "حالة السداد" (Payment Status) column showing `سُدِّد نقداً / سُدِّد بنكاً (PV-####)` that navigates to the linked voucher. General account overrides (cash, bank, payable defaults) live in the Accounting Mappings control panel under the `supplier_settlement` group ("تسوية الموردين (سندات الصرف)"). The backend defensively forces `entityType="supplier"`, IDOR-tightens all routes by `(id, companyId)`, and falls back to the mapping accounts when the cash/bank/supplier record itself has no `accountId`. The legacy `PaymentVouchers.tsx` page is now listing-only and navigates to the new full-page form.
