...
- **Developer Cloud / سحابة المطورين (Phase 5)**: SuperAdmin-only, fully additive module (`dev_workspaces`/`dev_workspace_seats`/`dev_deployments`) modelling a Replit-like cloud for partner teams. ONE isolated workspace per company (sandbox + git + storage + test env) on a managed provider — the platform stores only OPAQUE provider references (external IDs/URLs), NEVER credentials/SSH/RDP/DB. Multi-role developer seats (pm/backend/frontend/mobile/qa/devops) carry least-privilege permission maps (`DEV_ROLE_DEFAULT_PERMISSIONS`); `trigger_publish` is the ONLY deploy lever. Deployment is Publish-engine-only: `dev_deployments.method` is hard-fixed to `'publish_engine'`, the triggering seat must hold `trigger_publish`, and the workspace must be `active`. Routes `/api/admin/dev-cloud/*` (self-guarded by `requireSuperAdmin`, mounted before `zatcaRouter`), UI `/admin/dev-cloud` (SA-only route, no company module gate). Provisioning records references; real provider SDK calls would populate the same fields.
- **Brand Management (العلامة التجارية)**: additive/optional — one item, many brands (`brands`/`item_brands`), each brand carries own price/cost/barcode/part-number; reuses `item_groups` as categories. Brand picker in SALES INVOICES ONLY (`SalesDocumentForm`, `isInvoice && itemId`). `brandName` is PRINT-ONLY (snapshotted onto the invoice line, shown in `SalesPrintModal`) — it NEVER enters the ZATCA UBL XML / hash / QR / ICV-PIH (XML byte-identical brandId null vs set; "brand" absent from all `zatca-*` builders). Still validated server-side (`validateBrandLinesBelongToCompany`: brandId∈tenant + linked to line item, else normalized null). Master screen `/inventory/brands`, item links via `ItemBrandsPanel` tab, module gate `brands`. Routes `/api/brands/*`, `/api/items/:id/brands`.
...
- **DevStudio / التطوير من خلال زاكود**: SuperAdmin-governed, fully-additive in-browser AI coding studio where externally-registered+approved developers work on an ISOLATED, version-pinned codebase SNAPSHOT (safe model #1: read-only scoped access + AI-proposed diffs, NO untrusted code execution on our infra). Dedicated tables (`dev_studio_*`: packages/developers/visibility/snapshots+assignment/sessions/usage/audit/proposals). Developer identity is SEPARATE from tenant users — own bearer in `dev_studio_sessions` (localStorage `zatca_devstudio_token`), kill-switch checked every call (suspend revokes session live). File visibility is **Default-Deny** (developer sees ONLY SA-granted `allowedPrefixes` of the assigned published snapshot). Per-developer read/write LINE quotas are server-enforced; **every content-exposing path is metered** (direct file read AND AI propose = read; proposal create AND update = write, update re-meters by delta) and audited. AI orchestrator reuses `aiClient` (Gemini-free first) with a deterministic rule-based fallback. Snapshot engine captures a curated text-only source tree (gzip JSON blob). Honesty caveats: no Docker/VM sandbox → propose-only; analog hole mitigated via least-privilege scope+audit+NDA+watermark. **Phase 2 (review/approval gate, DONE)**: a submitted proposal requires MANUAL SuperAdmin approval before it flips to `published` (Option 1 — no auto-publish). An ADVISORY review engine (`proposalReview.ts`, diff-text-only, never applies/executes) runs gates scope (touched files within the developer's visibility allow-list, else fail) / stats / danger_scan (risky patterns in ADDED lines → warn) / ai_review (aiClient + deterministic rule-based fallback) → aggregate verdict + sha256 diffHash. Decision endpoint `/admin/dev-studio/proposals/:id/status` accepts only a `submitted` proposal (409 else), reject needs a reason (≥3), and persists a tamper-evident record (reviewReport/reviewVerdict/diffHash/reviewedBy/reviewedAt/decisionReason). `published` is a GOVERNANCE status only — actual code application to production stays a human/main-agent step. Routes `/api/dev-studio/*` (developer) + `/api/admin/dev-studio/*` (SA, `requireSuperAdmin`-guarded) mounted BEFORE `zatcaRouter`; public `/dev-studio/register` + `/dev-studio` studio (rendered ahead of auth gates), SA `/admin/dev-studio`.
