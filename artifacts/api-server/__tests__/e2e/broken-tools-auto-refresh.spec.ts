// E2E test for the SuperAdmin amber "أدوات صيانة تعطّلت آخر N أيام" panel
// auto-refresh on /admin/ai-fix in the zatca-invoicing artifact (task #96).
//
// What this verifies (mirrors the regression task acceptance criteria):
//   1. The "آخر تحديث: …" timestamp surfaced via
//      data-testid="error-summary-last-updated" advances on its own (no
//      reload, no user interaction) after the configured polling interval —
//      proving `errorSummaryQ` keeps `refetchInterval: MAINTENANCE_PANEL_
//      REFETCH_MS` (30s).
//   2. A NEW (company, tool) error row inserted into the DB while the page
//      is open appears in the panel without a manual reload — proving the
//      background refetch actually fetches fresh data and the UI re-renders
//      against it. Combined with (1), this catches the silent regression
//      the task is worried about (someone drops the refetchInterval and the
//      panel goes stale).
//
// This is the broken-tool sibling of critical-alerts-auto-refresh.spec.ts
// (task #83). The first-paint render contract is already covered by
// broken-tools-panel.spec.ts (task #85) — this spec exists to lock down
// the polling behaviour that first-paint tests can't observe.
//
// Determinism story:
//   - We seed a brand-new `companies` row tagged with a per-run TEST_TAG so
//     the seeded broken tools never collide with whatever else lives in the
//     shared dev DB.
...
//     may already have other broken tools on the page).
//   - `runAt` is set to ~1 day ago — comfortably inside the 7-day
//     TOOL_ERROR_WINDOW_DAYS so getRecentToolErrors picks the row up.
...
const TEST_TAG = `e2e_task96_${randomBytes(4).toString("hex")}`;
// Two distinct tool keys on the same seeded company exercise the per-
// (company, tool) latest projection in `getRecentToolErrors` (artifacts/
// api-server/src/lib/maintenanceScheduler.ts ~L828). TOOL_KEY_A is seeded
...
// newly broken tools without a manual reload.
const TOOL_KEY_A = `${TEST_TAG}_broken_a`;
const TOOL_KEY_B = `${TEST_TAG}_broken_b`;
// Distinctive error strings on the seeded rows so the panel's
// "رسالة الخطأ" cell can be uniquely matched if needed.
const ERROR_MSG_A = `boom A from task #96 e2e ${TEST_TAG}`;
const ERROR_MSG_B = `boom B from task #96 e2e ${TEST_TAG}`;
// Header text the broken-tool panel renders (AICompanyFix.tsx around
// line 1829). Matched as a prefix only because the same line interpolates
...
// ─── Setup: create company, sa_session, and the first broken-tool row ──────
test.beforeAll(async () => {
  ...
  await db
    ...
    .where(like(maintenanceRunsTable.toolKey, "e2e_task96_%"));
  await db
    .delete(superAdminSessionsTable)
    .where(like(superAdminSessionsTable.sessionToken, "e2e_task96_%"));
  await db
    .delete(companiesTable)
    .where(like(companiesTable.nameAr, "e2e_task96_%"));
  ...
  saSessionToken = "e2e_task96_" + randomBytes(16).toString("hex");
  const [s] = await db
  ...
  const [co] = await db
    ...
    .values({
      nameAr:         `${TEST_TAG} شركة الاختبار للأعطال المتجددة`,
      nameEn:         `${TEST_TAG} Test Co Broken Refresh`,
      vatNumber:      "300000000000096",
  ...
  // First broken-tool row — TOOL_KEY_A. The DISTINCT ON (company_id, tool_
  // key) projection in `getRecentToolErrors` will pick this up (no later
  // row exists for this pair) and the panel should render it on first
  // paint.
  //
  // runAt is set to "now" rather than backdated. The endpoint applies a
  // top-50 cap ordered by run_at DESC AFTER the per-(company, tool) latest
  // projection (artifacts/api-server/src/routes/admin.ts ~L4914 →
  // getRecentToolErrors at maintenanceScheduler.ts ~L828), so against a
  // shared dev DB with many older broken tools an older seeded row could
  // legitimately get ranked off the page. Seeding at current-time keeps
  // the row near the top of that ordering and matches the determinism
  // story used by the critical-alerts auto-refresh template.
  const aRow = await db
...
test("broken-tools panel auto-refresh: timestamp advances and a newly broken tool appears without reload", async ({ page }) => {
  ...
  // Locator scoped to the amber broken-tool panel via its header text.
  // Anchoring on the Arabic copy (PANEL_HEADER_PREFIX) keeps assertions
  // resilient to styling changes on the surrounding container (the
  // bg-amber-50/40 / border-amber-200 class chain could legitimately
  // move without breaking the contract we're testing).
  ...
  // the broken-tool projection on first paint, not just the header. We
  // anchor on the unique TOOL_KEY_A so the dev DB's own broken tools
  ...
  const timestamp = panel.locator('[data-testid="error-summary-last-updated"]');
  ...
  // While the page is open, raise a NEW broken-tool error for a different
  ...
  // poll below can fail fast if the panel never refetches. runAt is set
  // to "now" so the row falls inside the 7-day window and outranks
  // anything else for this (company, tool) pair.
  const bRow = await db
    .insert(maintenanceRunsTable)
    .values({
      companyId:  testCompanyId!,
  ...
  // error row into view. We give 60s — comfortably more than one tick —
