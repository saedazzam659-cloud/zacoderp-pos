...
export default function PaymentVoucherForm() {
  ...
  // Auto-dispatch: vouchers that carry allocation lines belong to the
  // multi-operation form (their accounts live in the lines, not the header).
  // Legacy/simple vouchers have no lines and stay on this simple form.
  useEffect(() => {
    if (!existing || !Array.isArray(existing.lines) || existing.lines.length === 0) return;
    if (editId != null) navigate(`/cash/payment-vouchers/multi/${editId}`, { replace: true });
    else if (fromId != null) navigate(`/cash/payment-vouchers/multi/new?from=${fromId}`, { replace: true });
  }, [existing, editId, fromId, navigate]);

  useEffect(() => {
    if (!existing) return;
    if (Array.isArray(existing.lines) && existing.lines.length > 0) return;
    setForm({
