...
const COLS = [
  ...
  { key: "nameAr",        header: "اسم الحساب",    width: 24 },
  { key: "bankName",      header: "البنك",         width: 18 },
  { key: "accountNumber", header: "رقم الحساب",    width: 18 },
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Landmark className="h-6 w-6 text-primary" />أرصدة البنوك</h1>
          <p className="text-muted-foreground text-sm mt-1">رصيد كل حساب بنكي بتاريخ محدد</p>
        </div>
        <ExportButtons
          rows={exportRows}
          columns={COLS}
          filename={`أرصدة-البنوك-${asOf}`}
          title="أرصدة البنوك"
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-semibold text-muted-foreground">البنك</th>
                <th className="px-4 py-3 text-right font-semibold text-muted-foreground">رقم الحساب</th>
            ...
            <tbody className="divide-y">
              {isLoading
                ...
                : data.length === 0
                ? <tr><td colSpan={7} className="py-12 text-center text-muted-foreground">لا توجد حسابات بنكية</td></tr>
                : data.map(r => (
