...
export default function PaymentVouchersReport() {
  const { fmt } = useFmt();
  const { t, i18n } = useTranslation();
  const isRtl = i18n.language === "ar";
  const tr = (k: string, opts?: any) => t(`cashReports.payments.${k}`, opts) as string;
  ...
  const COLS = [
    ...
    { key: "paymentTypeLabel", header: isRtl ? "نوع الدفع" : "Payment Type", width: 12 },
    { key: "entityTypeLabel",  header: isRtl ? "نوع الجهة" : "Entity Type",  width: 12 },
    { key: "entityName",       header: tr("payeeName"),     width: 22 },
    { key: "description",      header: tr("description"),   width: 28 },
    { key: "amount",           header: tr("amount"),        width: 16 },
  ];

  const { data = [], isLoading } = useQuery({
    queryKey: ["payment-vouchers-report", cid, from, to, paymentType, entityType, branchId],
    queryFn: () => cashAnalyticsApi.payments(cid, {
      from, to,
  ...
  return (
    <div className="space-y-6" dir={isRtl ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><ArrowUpCircle className="h-6 w-6 text-primary" />{tr("title")}</h1>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={paymentType} onValueChange={setPaymentType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="bank">{isRtl ? "بنكي" : "Bank"}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            <Label>{isRtl ? "نوع الجهة" : "Entity Type"}</Label>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border bg-rose-50 border-rose-200 p-4 col-span-2">
          <p className="text-xs text-rose-700">{tr("totalAmount")}</p>
          <p className="text-2xl font-bold text-rose-700 tabular-nums mt-1">{fmt(total)}</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className={`px-4 py-3 ${isRtl ? "text-right" : "text-left"} font-semibold text-muted-foreground`}>{isRtl ? "نوع الدفع" : "Payment Type"}</th>
                <th className={`px-4 py-3 ${isRtl ? "text-right" : "text-left"} font-semibold text-muted-foreground`}>{isRtl ? "نوع الجهة" : "Entity Type"}</th>
                <th className={`px-4 py-3 ${isRtl ? "text-right" : "text-left"} font-semibold text-muted-foreground`}>{tr("payeeName")}</th>
                <th className={`px-4 py-3 ${isRtl ? "text-right" : "text-left"} font-semibold text-muted-foreground`}>{tr("description")}</th>
                <th className="px-4 py-3 text-center font-semibold text-rose-700">{tr("amount")}</th>
              </tr>
            </thead>
              ...
              {isLoading
                ...
                : data.map(r => (
                    <tr key={r.id} className="hover:bg-muted/20">
                      ...
                      <td className="px-4 py-3 text-xs">{r.entityName ?? dash}</td>
                      <td className="px-4 py-3 text-xs text-muted-foreground">{r.description ?? dash}</td>
                      <td className="px-4 py-3 text-center tabular-nums text-sm font-bold text-rose-600">{fmt(r.amount)}</td>
                    </tr>
                  ))}
            </tbody>
            {!isLoading && data.length > 0 && (
              <tfoot className="bg-muted/30 border-t">
                <tr>
                  ...
                  <td className="px-4 py-3 text-center font-bold tabular-nums text-rose-700">{fmt(total)}</td>
                </tr>
              </tfoot>
            )}
          </table>
