...
const COLS = [
  ...
  { key: "entityName",       header: "المدفوع له",   width: 22 },
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><ArrowUpCircle className="h-6 w-6 text-primary" />تقرير سندات الصرف</h1>
          <p className="text-muted-foreground text-sm mt-1">قائمة بكل سندات الصرف المعتمدة خلال الفترة</p>
        </div>
        <ExportButtons
          rows={exportRows}
          columns={COLS}
          filename={`سندات-الصرف-${from}-${to}`}
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-semibold text-muted-foreground">المدفوع له</th>
                <th className="px-4 py-3 text-right font-semibold text-muted-foreground">البيان</th>
                <th className="px-4 py-3 text-center font-semibold text-rose-700">المبلغ</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {isLoading
                ...
                : data.length === 0
                ? <tr><td colSpan={7} className="py-12 text-center text-muted-foreground">لا توجد سندات صرف في الفترة المحددة</td></tr>
                : data.map(r => (
