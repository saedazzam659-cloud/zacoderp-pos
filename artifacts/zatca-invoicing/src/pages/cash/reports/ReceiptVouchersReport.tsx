...
export default function ReceiptVouchersReport() {
  ...
  const tr = (k: string, opts?: any) => t(`cashReports.receipts.${k}`, opts) as string;
  ...
  const COLS = [
    ...
    { key: "entityName",       header: tr("payerName"),     width: 22 },
  ...
  return (
    <div className="space-y-6" dir={isRtl ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><ArrowDownCircle className="h-6 w-6 text-primary" />{tr("title")}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border bg-emerald-50 border-emerald-200 p-4 col-span-2">
          <p className="text-xs text-emerald-700">{tr("totalAmount")}</p>
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className={`px-4 py-3 ${isRtl ? "text-right" : "text-left"} font-semibold text-muted-foreground`}>{tr("payerName")}</th>
                <th className={`px-4 py-3 ${isRtl ? "text-right" : "text-left"} font-semibold text-muted-foreground`}>{tr("description")}</th>
                <th className="px-4 py-3 text-center font-semibold text-emerald-700">{tr("amount")}</th>
              </tr>
            </thead>
