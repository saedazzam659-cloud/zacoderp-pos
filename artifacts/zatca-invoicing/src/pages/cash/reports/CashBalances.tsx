...
export default function CashBalances() {
  ...
  const COLS = [
    ...
    { key: "name",     header: t("cashReports.cashBalances.cashBoxName"), width: 24 },
  ...
  return (
    <div className="space-y-6" dir={isRtl ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Wallet className="h-6 w-6 text-primary" />{t("cashReports.cashBalances.title")}</h1>
          <p className="text-muted-foreground text-sm mt-1">{t("cashReports.cashBalances.subtitle")}</p>
        </div>
        <ExportButtons
          rows={exportRows}
          columns={COLS}
          filename={`${t("cashReports.cashBalances.filename")}-${asOf}`}
          title={t("cashReports.cashBalances.exportTitle")}
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className={`px-4 py-3 ${isRtl ? "text-right" : "text-left"} font-semibold text-muted-foreground`}>{t("cashReports.cashBalances.cashBoxName")}</th>
            ...
            <tbody className="divide-y">
              {isLoading
                ...
                : data.length === 0
                ? <tr><td colSpan={5} className="py-12 text-center text-muted-foreground">{t("cashReports.cashBalances.noData")}</td></tr>
                : data.map(r => (
            ...
            {!isLoading && data.length > 0 && (
              <tfoot className="bg-muted/30 border-t">
                <tr>
                  <td colSpan={2} className="px-4 py-3 text-xs font-semibold text-muted-foreground">{t("cashReports.common.totalRow")}</td>
