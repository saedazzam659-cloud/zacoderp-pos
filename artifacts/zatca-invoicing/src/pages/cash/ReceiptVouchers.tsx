...
import { ArrowDownCircle, Plus, Pencil, Trash2, Search, CheckCircle2, Clock, Send, Save, X, ListChecks } from "lucide-react";
...
export default function ReceiptVouchers() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { toast({ title: editing ? "تم التحديث" : "تم إنشاء السند" }); qc.invalidateQueries({ queryKey: ["receipt-vouchers"] }); resetForm(); },
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => { toast({ title: "تم الحذف" }); qc.invalidateQueries({ queryKey: ["receipt-vouchers"] }); setDelRow(null); if (editing && editing.id === delRow?.id) resetForm(); },
  ...
  return (
    <div className="space-y-4" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            { label: "إجمالي المقبوضات", value: totalAmount.toLocaleString("ar-SA-u-nu-latn", { minimumFractionDigits: 2 }), color: "text-emerald-700" },
          ].map((s, i) => (
      ...
      {/* Split layout: Form (right) + List (left) */}
          ...
          <div className="flex items-center justify-between border-b bg-muted/30 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold">
                {editing ? `تعديل سند #${editing.code}` : "سند قبض جديد"}
              </p>
            </div>
            {editing && (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {isLoading ? Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i} className="border-b"><td colSpan={6} className="px-3 py-3"><Skeleton className="h-4 w-full" /></td></tr>
                )) : filtered.length === 0 ? (
                  ...
                  <tr key={row.id} className={`border-b hover:bg-muted/20 transition-colors cursor-pointer ${editing?.id === row.id ? "bg-primary/5" : ""}`} onClick={() => row.status === "draft" && loadEdit(row)}>
                    ...
                    <td className="px-3 py-2.5 max-w-48">
                      <p className="text-xs font-medium truncate">{row.entityName || ENTITY_LABELS[row.entityType] || "—"}</p>
                      {row.description && <p className="text-[11px] text-muted-foreground truncate">{row.description}</p>}
                    </td>
                    <td className="px-3 py-2.5 hidden lg:table-cell">
                      <span className={`text-[11px] px-2 py-0.5 rounded-full ${row.paymentType === "cash" ? "bg-amber-50 text-amber-700" : "bg-blue-50 text-blue-700"}`}>
                        {row.paymentType === "cash" ? "نقداً" : "بنك"}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 font-medium text-green-700 tabular-nums text-xs">
                      {parseFloat(row.amount || "0").toLocaleString("ar-SA-u-nu-latn", { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      {row.status === "posted"
                        ? <span className="inline-flex items-center gap-1 text-[10px] text-green-700 bg-green-50 border border-green-200 px-1.5 py-0.5 rounded-full"><CheckCircle2 className="h-3 w-3" />مرحّل</span>
                        : <span className="inline-flex items-center gap-1 text-[10px] text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full"><Clock className="h-3 w-3" />مسودة</span>}
                    </td>
