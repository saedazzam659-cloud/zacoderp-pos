...
// Clamp every element into a uniform safe area inside the page so presets
// never paint anything against (or past) the page edge. Width/height are
// reduced first, then x/y are pushed in so the element's right/bottom edge
// stays within (pageW - safe, pageH - safe). All presets pipe their
// elements through this before returning.
function fit(elements: Element[], pageW: number, pageH: number, safe: number): Element[] {
  const maxW = pageW - safe * 2;
  const maxH = pageH - safe * 2;
  return elements.map(el => {
    ...
    if (x < safe) x = safe;
    if (y < safe) y = safe;
    if (x + width  > pageW - safe) x = pageW - safe - width;
    if (y + height > pageH - safe) y = pageH - safe - height;
