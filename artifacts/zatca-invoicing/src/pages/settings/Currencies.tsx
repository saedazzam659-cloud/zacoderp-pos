...
import {
  Plus, Pencil, Trash2, Search, X, DollarSign,
  ArrowRightLeft, Star, CheckCircle2, RefreshCw,
  Calendar,
} from "lucide-react";
...
export default function Currencies() {
  ...
  const createCur = useMutation({
    mutationFn: (d: any) => currenciesApi.create({ ...d, companyId: cid }),
    onSuccess: () => { inv(); resetCur(); toast({ title: "تمت إضافة العملة" }); },
    onError: (e: any) => toast({ title: "خطأ", description: e.message, variant: "destructive" }),
  });
  const updateCur = useMutation({
    mutationFn: ({ id, data }: any) => currenciesApi.update(id, data),
    onSuccess: () => { inv(); resetCur(); toast({ title: "تم تعديل العملة" }); },
    onError: (e: any) => toast({ title: "خطأ", description: e.message, variant: "destructive" }),
  });
  const deleteCur = useMutation({
    mutationFn: (id: number) => currenciesApi.remove(id),
    onSuccess: () => { inv(); setDeleteId(null); toast({ title: "تم الحذف" }); },
    onError: (e: any) => { setDeleteId(null); toast({ title: "خطأ", description: e.message, variant: "destructive" }); },
  });

  const createRate = useMutation({
    mutationFn: (d: any) => currenciesApi.createRate({ ...d, companyId: cid }),
    onSuccess: () => { inv(); resetRate(); toast({ title: "تمت إضافة معامل التحويل" }); },
    onError: (e: any) => toast({ title: "خطأ", description: e.message, variant: "destructive" }),
  });
  const updateRate = useMutation({
    mutationFn: ({ id, data }: any) => currenciesApi.updateRate(id, data),
    onSuccess: () => { inv(); resetRate(); toast({ title: "تم تعديل معامل التحويل" }); },
    onError: (e: any) => toast({ title: "خطأ", description: e.message, variant: "destructive" }),
  });
  const deleteRate = useMutation({
    mutationFn: (id: number) => currenciesApi.removeRate(id),
    onSuccess: () => { inv(); setDeleteRateId(null); toast({ title: "تم الحذف" }); },
  });
  ...
  function handleEditRate(r: any) {
    setRateForm({
      fromCurrencyId: String(r.fromCurrencyId),
      toCurrencyId:   String(r.toCurrencyId),
  ...
  function getCurName(id: number) {
    const c = currencies.find((c: any) => c.id === id);
    return c ? `${c.symbol ?? ""} ${c.nameAr} (${c.code})` : `#${id}`;
  }
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} dir="rtl">
        <Card className="border-2">
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/20">
              <p className="text-[11px] text-muted-foreground">
                {activeTab === "currencies"
                  ? `${filtered.length} عملة مسجّلة`
                  : `${rates.length} معامل تحويل مسجّل`}
              </p>
              <div className="flex items-center gap-2">
                {activeTab === "currencies" && (
                  <Button
                    size="sm" className="h-7 gap-1 text-xs"
                    onClick={() => { resetCur(); setShowCurForm(true); }}
                  >
                ...
                {activeTab === "rates" && (
                  <Button
                    size="sm" className="h-7 gap-1 text-xs"
                    onClick={() => { resetRate(); setShowRateForm(true); }}
                ...
                <TabsList className="h-8 bg-background border gap-1">
                  <TabsTrigger value="currencies" className="h-7 px-3 text-xs gap-1.5 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                    ...
                    العملات
                    {currencies.length > 0 && (
                      <span className="mr-1 bg-primary-foreground/20 rounded-full px-1.5 text-[10px] font-bold">
                        {currencies.length}
                      </span>
                    )}
                  </TabsTrigger>
          ...
          <TabsContent value="currencies" className="mt-0">
            <div className={cn("grid gap-0", showCurForm ? "grid-cols-[1fr_380px]" : "grid-cols-1")}>

              {/* Left: list */}
              <div className={cn(showCurForm && "border-l")}>
                {/* Search */}
                ...
                {isLoading ? (
                  ...
                  <div className="py-16 text-center space-y-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => setShowCurForm(true)}>
                      <Plus className="h-3.5 w-3.5 ml-1" /> إضافة عملة
                    </Button>
                  </div>
                ) : (
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/30 text-muted-foreground">
                        ...
                        <th className="px-4 py-2.5 text-right font-medium">الاسم الإنجليزي</th>
                      ...
                      {filtered.map((c: any) => (
                        <tr
                          ...
                          className={cn(
                            ...
                            curEditId === c.id && "bg-primary/5 border-r-2 border-primary"
                          )}
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-1">
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary" onClick={() => handleEditCur(c)}>
              ...
              {showCurForm && (
                <div className="bg-muted/10">
                  <div className="flex items-center justify-between px-4 py-3 border-b">
                    <h3 className="text-sm font-semibold">{curEditId ? "تعديل العملة" : "إضافة عملة جديدة"}</h3>
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={resetCur}>
                      <X className="h-3.5 w-3.5" />
                    </Button>
                  </div>

                  <form onSubmit={submitCur} className="p-4 space-y-4">
                    {/* Quick-fill from common */}
                    {!curEditId && (
                      <div>
                        <p className="text-[11px] font-medium text-muted-foreground mb-2">عملات شائعة:</p>
                          ...
                          {COMMON_CURRENCIES.map(c => {
                            ...
                            return (
                              <button
                                key={c.code}
                                type="button"
                                onClick={() => fillFromCommon(c)}
                                disabled={exists}
                                className={cn(
                                  "text-[11px] px-2 py-1 rounded border font-mono font-bold transition-colors",
                                  exists
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <Label className="text-xs font-medium">الكود <span className="text-destructive">*</span></Label>
                        <Input
                          value={curForm.code}
                          onChange={e => setCurForm((p: any) => ({ ...p, code: e.target.value.toUpperCase() }))}
                          placeholder="SAR"
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={e => setCurForm((p: any) => ({ ...p, symbol: e.target.value }))}
                          placeholder="ر.س"
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-medium">الاسم العربي <span className="text-destructive">*</span></Label>
                      <Input
                        value={curForm.nameAr}
                        onChange={e => setCurForm((p: any) => ({ ...p, nameAr: e.target.value }))}
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-medium">الاسم الإنجليزي</Label>
                      <Input
                        value={curForm.nameEn}
                        onChange={e => setCurForm((p: any) => ({ ...p, nameEn: e.target.value }))}
                    ...
                    <div className="flex items-center gap-4 pt-1">
                      <div className="flex items-center gap-2">
                        <Switch
                          id="isDefault"
                          checked={curForm.isDefault}
                          onCheckedChange={v => setCurForm((p: any) => ({ ...p, isDefault: v }))}
                        />
                        <Label htmlFor="isDefault" className="text-xs cursor-pointer flex items-center gap-1">
                          <Star className="h-3 w-3 text-yellow-500" /> افتراضية
                        </Label>
                      </div>
                      <div className="flex items-center gap-2">
                        <Switch
                          id="isActive"
                          checked={curForm.isActive}
                          onCheckedChange={v => setCurForm((p: any) => ({ ...p, isActive: v }))}
                        />
                        <Label htmlFor="isActive" className="text-xs cursor-pointer flex items-center gap-1">
                          <CheckCircle2 className="h-3 w-3 text-green-500" /> نشطة
                        </Label>
                      </div>
                    </div>

                    <div className="flex gap-2 pt-2">
                      <Button type="submit" size="sm" className="flex-1" disabled={createCur.isPending || updateCur.isPending}>
                        {(createCur.isPending || updateCur.isPending) ? "جارٍ الحفظ..." : curEditId ? "تعديل" : "إضافة"}
                      </Button>
                      <Button type="button" variant="outline" size="sm" onClick={resetCur}>إلغاء</Button>
                    </div>
                  </form>
                </div>
              )}
          ...
          <TabsContent value="rates" className="mt-0">
            ...
            {currencies.length >= 2 && (
              <div className={cn("grid gap-0", showRateForm ? "grid-cols-[1fr_380px]" : "grid-cols-1")}>

                {/* Left: rates list */}
                <div className={cn(showRateForm && "border-l")}>
                  {rates.length === 0 ? (
                    <div className="py-16 text-center space-y-2">
                        ...
                        {rates.map((r: any) => {
                          ...
                          return (
                            <tr key={r.id} className={cn(
                              "hover:bg-muted/20 transition-colors",
                              rateEditId === r.id && "bg-primary/5 border-r-2 border-primary"
                            )}>
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                              ...
                              <td className="px-4 py-3">
                                <span className="font-mono font-bold text-base text-foreground">{Number(r.rate).toFixed(4)}</span>
                              </td>
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center justify-center gap-1">
                                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary" onClick={() => handleEditRate(r)}>
                                    <Pencil className="h-3.5 w-3.5" />
                                  </Button>
                                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => setDeleteRateId(r.id)}>
                                    <Trash2 className="h-3.5 w-3.5" />
                                  </Button>
                                </div>
                              </td>
                ...
                {/* Right: rate form panel */}
                {showRateForm && (
                  <div className="bg-muted/10">
                    <div className="flex items-center justify-between px-4 py-3 border-b">
                      <h3 className="text-sm font-semibold">{rateEditId ? "تعديل معامل التحويل" : "إضافة معامل تحويل"}</h3>
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={resetRate}>
                        <X className="h-3.5 w-3.5" />
                      </Button>
                    </div>

                    <form onSubmit={submitRate} className="p-4 space-y-4">
                      <div className="space-y-1.5">
                        <Label className="text-xs font-medium">من العملة <span className="text-destructive">*</span></Label>
                        <Select
                          value={rateForm.fromCurrencyId || "__none"}
                          onValueChange={v => setRateForm((p: any) => ({ ...p, fromCurrencyId: v === "__none" ? "" : v }))}
                        >
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-xs font-medium">إلى العملة <span className="text-destructive">*</span></Label>
                        <Select
                          value={rateForm.toCurrencyId || "__none"}
                          onValueChange={v => setRateForm((p: any) => ({ ...p, toCurrencyId: v === "__none" ? "" : v }))}
                        >
                          <SelectTrigger className="h-9 text-sm">
                            <SelectValue placeholder="— اختر العملة —" />
                          </SelectTrigger>
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-xs font-medium flex items-center gap-1">
                          <Calendar className="h-3.5 w-3.5" />
                          تاريخ التطبيق <span className="text-destructive">*</span>
                        </Label>
                        <Input
                          type="date"
                          value={rateForm.effectiveDate}
                          onChange={e => setRateForm((p: any) => ({ ...p, effectiveDate: e.target.value }))}
                          className="h-9 text-sm"
                        />
                      </div>

                      <div className="flex gap-2 pt-1">
                        <Button type="submit" size="sm" className="flex-1" disabled={createRate.isPending || updateRate.isPending}>
                          {(createRate.isPending || updateRate.isPending) ? "جارٍ الحفظ..." : rateEditId ? "تعديل" : "إضافة"}
                        </Button>
                        <Button type="button" variant="outline" size="sm" onClick={resetRate}>إلغاء</Button>
                      </div>
                    </form>
                  </div>
                )}
        ...
        <AlertDialogContent dir="rtl">
          ...
          <AlertDialogFooter className="flex-row-reverse gap-2">
            ...
            <AlertDialogAction
              ...
              onClick={() => deleteId && deleteCur.mutate(deleteId)}
