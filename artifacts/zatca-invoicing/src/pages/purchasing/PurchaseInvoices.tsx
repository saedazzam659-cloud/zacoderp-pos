...
export default function PurchaseInvoices() {
  ...
  const postMut = useMutation({
    ...
    onSuccess: () => { invalidate(); toast({ title: "✓ تم ترحيل الفاتورة" }); },
  ...
  const filtered = invoices.filter(inv => {
    ...
    const matchText = !search || (inv.docNumber ?? "").includes(q) || (supMap[inv.supplierId] ?? "").includes(search);
    const matchStatus = filterStatus === "all" || inv.status === filterStatus;
    return matchText && matchStatus;
  });

  const totalDraft  = invoices.filter(i => i.status === "draft").reduce((s, i) => s + Number(i.totalAmount || 0), 0);
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <ShoppingCart className="h-6 w-6 text-primary" />فواتير المشتريات
          </h1>
          <p className="text-sm text-muted-foreground mt-1">إدارة فواتير المشتريات وتحميل مصاريف الاستيراد</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "إجمالي المشتريات", value: `${fmt(totalPosted)} ريال`, color: "text-primary", wide: true },
        ].map((c, i) => (
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-48">
          ...
          <Input className="pr-9" placeholder="بحث برقم الفاتورة أو المورد..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      ...
      <div className="rounded-xl border bg-card overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-muted-foreground text-sm">لا توجد فواتير مشتريات</p>
