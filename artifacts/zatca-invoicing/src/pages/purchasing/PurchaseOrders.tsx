...
export default function PurchaseOrders() {
  ...
  }, [orders, tableSearch, statusFilter, colAdv, supMap]);
                ...
                {pagedOrders.map((ord: any, idx: number) => {
                  ...
                  const renderCell = (col: ColDef) => {
                    switch (col.key) {
                      ...
                      case "netAfterDiscount":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-end font-mono text-slate-800">{fmt(Number(ord.totalAmount ?? 0) - Number(ord.vatAmount ?? 0))}</td>;
                      case "invoice":
                        return (
                  ...
                  return (
                    <tr key={ord.id}
                      ...
                      onDoubleClick={() => navigate(`/purchasing/orders/${ord.id}?view=1`)}
