...
export default function SupplierSettlement() {
  ...
  const supplierItems = [{ value: "", label: "— اختر المورد —" }, ...suppliers.map((s: any) => ({ value: String(s.id), label: s.nameAr }))];
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Banknote className="h-6 w-6 text-primary" />تسوية الموردين</h1>
          <p className="text-sm text-muted-foreground mt-1">سداد مستحقات الموردين وترحيل القيود</p>
        </div>
      ...
      {showForm && (
        <FormPanel
          icon={Banknote}
          title="تسوية جديدة"
          subtitle="سداد مستحقات المورد عبر بنك أو نقد أو شيك"
          width="4xl"
          onClose={reset}
        ...
        >
          <FormGrid>
            <Field label="رقم المستند"><Input placeholder="تلقائي" value={form.docNumber} onChange={e => setForm((p: any) => ({ ...p, docNumber: e.target.value }))} /></Field>
            ...
            <Field label="المورد" required className="md:col-span-2">
      ...
      <div className="rounded-xl border bg-card overflow-hidden shadow-sm">
        {isLoading ? <div className="p-12 text-center text-muted-foreground text-sm">جاري التحميل...</div>
          : settlements.length === 0 ? <div className="p-12 text-center text-muted-foreground text-sm">لا توجد تسويات بعد</div>
          : (
          <table className="w-full text-sm">
            <thead><tr className="bg-muted/50 border-b">
              {["رقم المستند","التاريخ","المورد","طريقة الدفع","الحساب","المبلغ","العملة","الحالة","إجراءات"].map(h =>
                    ...
                    <div className="flex gap-1">
                      {s.status === "draft" && (
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-green-700" title="ترحيل"
                          onClick={() => { if (confirm("ترحيل التسوية؟")) postMut.mutate(s.id); }}>
                          <CheckCircle className="h-3.5 w-3.5" />
                        </Button>
                      )}
                      {s.status === "draft" && (
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive"
                          onClick={() => { if (confirm("حذف التسوية؟")) deleteMut.mutate(s.id); }}>
