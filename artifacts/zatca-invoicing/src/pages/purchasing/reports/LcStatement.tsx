...
export default function LcStatement() {
  ...
  const totalsRow = summary && rows.length > 0 ? {
    lcNumber: "", lcDate: "", supplier: "", bank: "", currency: "", rate: "",
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[1100px]">
            ...
            <tbody className="divide-y">
              {isLoading
                ? [...Array(5)].map((_, i) => <tr key={i}><td colSpan={12} className="px-4 py-3"><Skeleton className="h-6 w-full" /></td></tr>)
                : rows.length === 0
                ? <tr><td colSpan={12} className="py-12 text-center text-muted-foreground">{tr("noData")}</td></tr>
                : rows.map(lc => {
            ...
            {!isLoading && rows.length > 0 && summary && (
              <tfoot className="bg-muted/40 border-t-2">
                <tr>
                  <td colSpan={6} className="px-3 py-3 text-sm font-semibold">{t("purchasingPages.common.total")}</td>
