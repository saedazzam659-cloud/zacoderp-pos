...
const EXPORT_COLS = [
  { key: "supplierNameAr", header: "المورد",         width: 28 },
...
export default function SupplierAgingReport() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["supplier-aging", cid, asOf],
    queryFn: () => purchaseAnalyticsApi.aging(cid, asOf),
  });

  const filtered = (rows as any[]).filter(r =>
    !search || r.supplierNameAr?.includes(search) || r.phone?.includes(search)
  );
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><AlertTriangle className="h-6 w-6 text-amber-500" />أعمار الذمم الدائنة (الموردين)</h1>
          <p className="text-muted-foreground text-sm mt-1">توزيع المستحقات للموردين على فترات زمنية لتحديد أولويات الدفع</p>
        </div>
        <ExportButtons
          rows={exportRows}
          columns={EXPORT_COLS}
          filename={`اعمار-الموردين-${asOf}`}
          title="تقرير أعمار الذمم الدائنة"
          subtitle={`بتاريخ ${asOf}  |  إجمالي المستحقات: ${fmt(totals.total)} ر.س`}
        />
      </div>
          ...
          <table className="w-full text-sm min-w-[820px]">
            ...
            <tbody className="divide-y">
              {isLoading
                ...
                : filtered.length === 0
                ? <tr><td colSpan={8} className="py-12 text-center text-muted-foreground">لا توجد مستحقات قائمة للموردين</td></tr>
                : filtered.map((r: any) => (
            ...
            {!isLoading && filtered.length > 0 && (
              <tfoot className="bg-muted/30 border-t">
                <tr>
                  <td colSpan={2} className="px-3 py-3 text-xs font-bold">الإجمالي ({filtered.length} مورد)</td>
