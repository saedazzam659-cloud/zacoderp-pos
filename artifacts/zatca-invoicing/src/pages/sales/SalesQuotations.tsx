  ...
  const filteredQuotations = useMemo(() => {
    ...
    return (quotations as any[]).filter((qt) => {
      ...
      for (const col of COLUMNS) {
        ...
        if (!f) continue;
        if (!matchCol(col.valueOf(qt), f, col.type)) return false;
      }
      return true;
    });
  }, [quotations, tableSearch, statusFilter, colFilters, cusMap]);
...
<script>setTimeout(()=>window.print(),350);</script></body></html>`;
  ...
  async function handleBulkPrint() {
    ...
    try {
      ...
      const ordered = (filteredQuotations as any[]).filter((r) => idSet.has(Number(r.id)));
      let failed = 0;
      const docs = await Promise.all(
        ordered.map(async (row: any) => {
          try {
            const res = await fetch(`${API}/api/sales/sales-quotations/${row.id}${cid ? `?companyId=${cid}` : ""}`, { headers: authH });
      ...
      if (failed > 0) {
        toast({ title: "تعذّر تحميل تفاصيل بعض العروض", description: `تمت طباعة ${docs.length} مع ${failed} بدون بنود تفصيلية`, variant: "destructive" });
      }
