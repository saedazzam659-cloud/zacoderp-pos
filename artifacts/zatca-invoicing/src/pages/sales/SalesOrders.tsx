...
import {
  Plus, ClipboardList, Eye, Trash2, Copy, Pencil,
...
export default function SalesOrders() {
  ...
  const COLUMNS: ColDef[] = [
    ...
    { key: "doc",       label: t("salesOrders.colNumber"),          type: "text", valueOf: (r) => r.docNumber ?? `SO-${r.id}` },
    { key: "date",      label: t("salesOrders.colDate"),            type: "text", valueOf: (r) => r.orderDate ?? "" },
    { key: "delivery",  label: t("salesOrders.colExpectedDelivery"),type: "text", valueOf: (r) => r.expectedDeliveryDate ?? "" },
    { key: "customer",  label: t("salesOrders.colCustomer"),        type: "text", valueOf: (r) => cusMap[r.customerId] ?? "" },
    { key: "currency",  label: t("salesOrders.colCurrency"),        type: "text", valueOf: (r) => r.currencyCode ?? "" },
    { key: "subtotal",  label: t("salesOrders.colSubtotal"),        type: "num",  valueOf: (r) => Number(r.subtotal ?? 0) },
    { key: "vat",       label: t("salesOrders.colVat"),             type: "num",  valueOf: (r) => Number(r.vatAmount ?? 0) },
    { key: "total",     label: t("salesOrders.colTotal"),           type: "num",  valueOf: (r) => Number(r.totalAmount ?? 0) },
    { key: "netAfterDiscount", label: t("common.netAfterDiscount"), type: "num", valueOf: (r) => Number(r.totalAmount ?? 0) - Number(r.vatAmount ?? 0) },
    { key: "status",    label: t("salesOrders.colStatus"),          type: "text", valueOf: (r) => statusLabel(r.status) },
    { key: "_act",      label: t("salesOrders.colActions"),         type: "none", valueOf: () => "" },
  ];
  const DATA_KEYS = COLUMNS.filter(c => c.key !== "_sel" && c.key !== "_idx" && c.key !== "_act").map(c => c.key);
  const ALL_KEYS  = COLUMNS.map(c => c.key);

  const layout = useAuditGridLayout({
    screenSlug: "salesOrdersAuditGrid",
    cid,
    dataKeys: DATA_KEYS,
    allColKeys: ALL_KEYS,
  });
  ...
  const filteredOrders = useMemo(() => {
    ...
    return (orders as any[]).filter((o) => {
      ...
      if (q) {
        const hay = [
          o.docNumber, `SO-${o.id}`, o.orderDate, cusMap[o.customerId],
  ...
  const buildListHtml = (source: any[] = filteredOrders) => {
    ...
    return `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>${escapeHtml(t("salesOrders.title"))}</title>
<style>
...
<div class="h">${logoHtml}${companyHtml}<h1>${escapeHtml(t("salesOrders.title"))}</h1>
...
<table><thead><tr>
  <th>#</th><th>رقم الأمر</th><th>التاريخ</th><th>تاريخ التسليم</th><th>العميل</th>
  <th>العملة</th><th>المجموع</th><th>الضريبة</th><th>الإجمالي</th><th>الحالة</th>
</tr></thead><tbody>
${source.map((r: any, i: number) => `<tr>
  <td>${i + 1}</td>
  <td>${escapeHtml(r.docNumber ?? `SO-${r.id}`)}</td>
...
<script>setTimeout(()=>window.print(),300);</script></body></html>`;
  ...
  const buildBulkHtml = (docs: any[]) => {
    ...
    const sections = docs.map((d: any) => {
      ...
      const docNo  = d.docNumber ?? `SO-${d.id}`;
    ...
    return `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>طباعة أوامر البيع المحدّدة</title>
<style>
...
.b-status.s-confirmed{background:#d1fae5;border-color:#34d399;color:#065f46;}
...
<button class="print-btn" onclick="window.print()">طباعة</button>
<div class="h">${logoHtml}${companyHtml}<h1>أوامر البيع المحدّدة</h1>
...
<script>setTimeout(()=>window.print(),350);</script></body></html>`;
  ...
  const handleExportExcel = () => {
    ...
    ws["!cols"] = [{ wch: 14 }, { wch: 12 }, { wch: 12 }, { wch: 22 }, { wch: 8 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 10 }, { wch: 16 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "أوامر البيع");
    void saveWorkbook(wb, `sales-orders-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  function exportCsv() {
    ...
    downloadCsv(`sales-orders-${new Date().toISOString().slice(0, 10)}.csv`, header, rows);
  ...
  const selectedCancellable = selectedRows.filter((r) => r.status !== "converted" && r.status !== "cancelled");
  const selectedDeletable   = selectedRows.filter((r) => !r.convertedInvoiceId);
  ...
  async function bulkConfirm() {
    ...
    try {
      const { ok, failed } = await bulkRun(ids, async (id) => {
        const res = await fetch(`${API}/api/sales/sales-orders/${id}/status`, { method: "PATCH", headers, body: JSON.stringify({ status: "confirmed" }) });
        if (!res.ok) { const j = await res.json().catch(() => ({})); throw new Error(j?.error ?? `HTTP ${res.status}`); }
      });
      invalidate();
      if (failed.length === 0) toast({ title: `تم تأكيد ${ok} أمر بيع` });
  ...
  async function bulkCancel() {
    ...
    if (ids.length === 0) { toast({ title: "لا توجد أوامر قابلة للإلغاء ضمن المحدَّد", variant: "destructive" }); return; }
    if (!window.confirm(`إلغاء ${ids.length} أمر بيع؟`)) return;
    setBulkBusy(true);
    try {
      const { ok, failed } = await bulkRun(ids, async (id) => {
        const res = await fetch(`${API}/api/sales/sales-orders/${id}/status`, { method: "PATCH", headers, body: JSON.stringify({ status: "cancelled" }) });
        if (!res.ok) { const j = await res.json().catch(() => ({})); throw new Error(j?.error ?? `HTTP ${res.status}`); }
      });
      invalidate();
  ...
  async function bulkDelete() {
    ...
    if (ids.length === 0) { toast({ title: "لا يمكن حذف الأوامر المرتبطة بفواتير", variant: "destructive" }); return; }
        ...
        <AuditGridBulkBar count={layout.selected.size} onClear={clearSelection} busy={bulkBusy}>
          ...
          <Button type="button" size="sm" variant="outline"
            ...
            onClick={() => {
              ...
              navigate(`/sales/orders/${Array.from(layout.selected)[0]}`);
            }}
          ...
          <Button type="button" size="sm" variant="outline"
            ...
            onClick={() => {
              ...
              navigate(`/sales/orders/new?from=${Array.from(layout.selected)[0]}`);
            }}
            disabled={bulkBusy || layout.selected.size !== 1}
          ...
          <Button type="button" size="sm" variant="outline"
            ...
            title={selectedCancellable.length === 0 ? "لا توجد أوامر قابلة للإلغاء" : `إلغاء ${selectedCancellable.length} أمر`}>
          ...
          <Button type="button" size="sm"
            ...
            title={selectedDeletable.length === 0
              ? "لا يمكن حذف الأوامر المرتبطة بفواتير"
        ...
        <div className="overflow-x-auto" style={{ maxHeight: "calc(100vh - 320px)" }}>
          {isLoading ? (
            ...
            <div className="p-12 text-center text-muted-foreground text-sm">
              {orders.length === 0 ? t("salesOrders.noOrders") : "لا توجد أوامر ضمن التصفية الحالية"}
            </div>
          ) : (
            <table ref={tableRef} className="w-full text-[11px] border-collapse" dir={isRtl ? "rtl" : "ltr"}>
              ...
              <tbody>
                {pagedOrders.map((o: any, idx: number) => {
                  const absIdx = pageSize === 0 ? idx : (safePage - 1) * pageSize + idx;
                  const stCls = STATUS_CLS[o.status] ?? STATUS_CLS.draft;
                  ...
                  const renderCell = (col: ColDef) => {
                    switch (col.key) {
                        ...
                        return (
                          <td key={col.key} className="px-2 py-1 border border-slate-200 text-center">
                            <RowSelectCheckbox
                              ...
                              ariaLabel={`تحديد الأمر ${o.docNumber ?? `SO-${rid}`}`}
                            />
                          </td>
                        );
                      case "_idx":
                      ...
                      case "doc":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 font-mono font-semibold text-primary text-center">{o.docNumber ?? `SO-${o.id}`}</td>;
                      case "date":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-center whitespace-nowrap text-slate-600">{o.orderDate}</td>;
                      case "delivery":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-center whitespace-nowrap text-slate-500">{o.expectedDeliveryDate ?? <span className="text-muted-foreground">{t("common.none")}</span>}</td>;
                      case "customer":
                        return <td key={col.key} className={cn("px-2 py-1 border border-slate-200 truncate", colWidths.customer ? "" : "max-w-[200px]")} title={cusMap[o.customerId] ?? ""}>{cusMap[o.customerId] ?? t("common.none")}</td>;
                      case "currency":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-center text-slate-500 font-mono">{o.currencyCode}</td>;
                      case "subtotal":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-end font-mono text-slate-800">{fmt(o.subtotal)}</td>;
                      case "vat":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-end font-mono text-amber-700">{fmt(o.vatAmount)}</td>;
                      case "total":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-end font-mono font-bold text-slate-900">{fmt(o.totalAmount)}</td>;
                      case "netAfterDiscount":
                        return <td key={col.key} className="px-2 py-1 border border-slate-200 text-end font-mono text-slate-800">{fmt(Number(o.totalAmount ?? 0) - Number(o.vatAmount ?? 0))}</td>;
                      case "status":
                        return (
                          <td key={col.key} className="px-2 py-1 border border-slate-200 text-center">
                            <span className={cn("inline-flex items-center text-[10px] rounded px-1.5 py-0.5 font-medium border", stCls)}>{statusLabel(o.status)}</span>
                            {o.convertedInvoiceId && (
                              <span className="block mt-0.5 text-[9px] text-muted-foreground">
                            ...
                            <div className="flex items-center justify-center gap-0.5">
                              ...
                              {o.status === "draft" && (
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-green-700"
                                  title={t("salesOrders.confirm")}
                                  onClick={(e) => { e.stopPropagation(); statusMut.mutate({ id: o.id, status: "confirmed" }); }}>
                              ...
                              {o.status === "confirmed" && (
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-primary"
                                  ...
                                  onClick={(e) => { e.stopPropagation(); convertMut.mutate(o.id); }}>
                                  <ArrowRightLeft className="h-3.5 w-3.5" />
                                </Button>
                              )}
                              {!isTerminal && (
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-amber-700"
                                  title={t("salesOrders.cancel")}
                                  onClick={(e) => { e.stopPropagation(); if (window.confirm(t("salesOrders.confirmCancel"))) statusMut.mutate({ id: o.id, status: "cancelled" }); }}>
                                  <XCircle className="h-3.5 w-3.5" />
                                </Button>
                              )}
                              {!o.convertedInvoiceId && (
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-rose-600 hover:bg-rose-50 hover:text-rose-700"
                                  ...
                                  onClick={(e) => { e.stopPropagation(); if (window.confirm(t("salesOrders.confirmDelete"))) deleteMut.mutate(o.id); }}>
                                  <Trash2 className="h-3.5 w-3.5" />
                                </Button>
                              )}
                            </div>
