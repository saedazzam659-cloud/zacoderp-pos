...
const EXPORT_COLS = [
  ...
  { key: "subtotal",       header: "صافي المبيعات",  width: 16 },
  { key: "vatAmount",      header: "ض.ق.م",          width: 14 },
  { key: "totalSales",     header: "إجمالي المبيعات", width: 16 },
  { key: "totalReturns",   header: "المرتجعات",      width: 14 },
  { key: "netSales",       header: "الصافي بعد المرتجع", width: 18 },
  { key: "totalPaid",      header: "المحصل",         width: 14 },
];
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Users className="h-6 w-6 text-primary" />المبيعات حسب العميل</h1>
          <p className="text-muted-foreground text-sm mt-1">إجمالي المبيعات والمرتجعات والمحصل لكل عميل خلال الفترة</p>
        </div>
        <ExportButtons
          rows={exportRows}
          columns={EXPORT_COLS}
          filename={`المبيعات-حسب-العميل-${from}-${to}`}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="rounded-xl border bg-emerald-50 border-emerald-200 p-3">
          <p className="text-[11px] text-emerald-700">صافي المبيعات</p>
        ...
        <div className="rounded-xl border bg-primary/5 border-primary/10 p-3">
          <p className="text-[11px] text-muted-foreground">المحصل</p>
          ...
          <table className="w-full text-sm min-w-[900px]">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className="px-3 py-3 text-center font-semibold text-blue-700">إجمالي المبيعات</th>
                ...
                <th className="px-3 py-3 text-center font-semibold text-muted-foreground">المحصل</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {isLoading
                ...
                : filtered.length === 0
                ? <tr><td colSpan={8} className="py-12 text-center text-muted-foreground">لا توجد مبيعات في هذه الفترة</td></tr>
