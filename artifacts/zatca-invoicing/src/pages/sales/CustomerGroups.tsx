...
const EMPTY = { code: "", nameAr: "", nameEn: "", notes: "", isActive: true };
...
export default function CustomerGroups() {
  ...
  const tr = (k: string, opts?: any) => t(`salesPages.customerGroups.${k}`, opts) as string;
  ...
  const { data: groups = [], isLoading } = useQuery<any[]>({
    queryKey: ["customer-groups", cid],
    queryFn: async () => {
      const url = cid ? `${API}/api/customers/customer-groups?companyId=${cid}` : `${API}/api/customers/customer-groups`;
  ...
  const createMut = useMutation({
    mutationFn: async (data: any) => {
      const res = await fetch(`${API}/api/customers/customer-groups`, { method: "POST", headers, body: JSON.stringify({ ...data, companyId: cid }) });
  ...
  const updateMut = useMutation({
    mutationFn: async ({ id, data }: any) => {
      const res = await fetch(`${API}/api/customers/customer-groups/${id}`, { method: "PUT", headers, body: JSON.stringify({ ...data, companyId: cid }) });
      const j = await res.json(); if (!res.ok) throw new Error(j.error);
      return j;
    },
  ...
  const deleteMut = useMutation({
    mutationFn: async (id: number) => {
      const url = cid ? `${API}/api/customers/customer-groups/${id}?companyId=${cid}` : `${API}/api/customers/customer-groups/${id}`;
          ...
          <FormGrid>
            ...
            <Field label={tr("fStatus")}>
              <div className="flex items-center gap-2 h-9">
                <Switch checked={form.isActive} onCheckedChange={v => setForm((p: any) => ({ ...p, isActive: v }))} id="cg-active" />
                <Label htmlFor="cg-active" className="text-sm">{form.isActive ? tr("stActive") : tr("stInactive")}</Label>
              </div>
            </Field>
