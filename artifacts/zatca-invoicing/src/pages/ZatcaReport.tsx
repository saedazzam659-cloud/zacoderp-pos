...
export default function ZatcaReport() {
  ...
  // ─── Pie chart data ──────────────────────────────────────────────────
  const pieData = [
    { name: "معتمدة", value: kpis.approved, color: COLORS.approved },
    { name: "مرفوضة", value: kpis.rejected, color: COLORS.rejected },
    { name: "بانتظار", value: kpis.pending, color: COLORS.pending },
  ].filter(d => d.value > 0);
  ...
  const fmtMoney = (v: number) => v.toLocaleString("ar-SA", { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  // ─── CSV Export ──────────────────────────────────────────────────────
  const exportCsv = () => {
    const headers = ["رقم الفاتورة", "التاريخ", "العميل", "الرقم الضريبي", "الإجمالي", "الضريبة", "حالة ZATCA", "UUID", "تاريخ الإرسال", "أسباب الرفض"];
    const lines = filtered.map(r => {
      ...
      const status = r.zatcaStatus === "approved" ? "معتمدة"
                   : r.zatcaStatus === "rejected" ? "مرفوضة"
                   : "بانتظار";
      const cells = [
        r.docNumber || `#${r.id}`,
        (r.invoiceDate || "").slice(0, 10),
        r.customerNameAr || "نقدي",
        r.customerVatNumber || "",
        Number(r.totalAmount || 0).toFixed(2),
        Number(r.vatAmount || 0).toFixed(2),
        status,
        r.zatcaUuid || "",
        r.zatcaSubmittedAt ? new Date(r.zatcaSubmittedAt).toLocaleString("ar-SA") : "",
        reasons,
    ...
    a.href = url;
    a.download = `zatca-report-${dateFrom}_to_${dateTo}.csv`;
    a.click();
    ...
    <div className="p-6 space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">تقرير فواتير ZATCA</h1>
            <p className="text-sm text-muted-foreground">
              تقرير شامل عن الفواتير المُرسلة إلى هيئة الزكاة (المعتمدة والمرفوضة)
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
            ...
            <div className="md:col-span-2">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="رقم الفاتورة، العميل، الرقم الضريبي، UUID..."
                  value={search}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-5">
            ...
            <div className="text-xs text-muted-foreground mt-1">في النطاق المختار</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200">
          <CardContent className="pt-5">
            <div className="flex items-center justify-between">
              <div className="text-xs text-emerald-700 dark:text-emerald-300">معتمدة</div>
              <CheckCircle2 className="h-4 w-4 text-emerald-500" />
            </div>
            <div className="text-2xl font-bold mt-1 text-emerald-600 dark:text-emerald-400">{kpis.approved}</div>
            <div className="text-xs text-emerald-700 dark:text-emerald-300 mt-1">{fmtMoney(kpis.approvedAmount)} ر.س</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-red-50/50 dark:bg-red-950/20 border-red-200">
          <CardContent className="pt-5">
            <div className="flex items-center justify-between">
              <div className="text-xs text-red-700 dark:text-red-300">مرفوضة</div>
              <XCircle className="h-4 w-4 text-red-500" />
            </div>
            <div className="text-2xl font-bold mt-1 text-red-600 dark:text-red-400">{kpis.rejected}</div>
            <div className="text-xs text-red-700 dark:text-red-300 mt-1">{fmtMoney(kpis.rejectedAmount)} ر.س</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-indigo-50/50 dark:bg-indigo-950/20 border-indigo-200">
          <CardContent className="pt-5">
            <div className="flex items-center justify-between">
              <div className="text-xs text-indigo-700 dark:text-indigo-300">معدّل القبول</div>
            ...
            <div className="text-xs text-indigo-700 dark:text-indigo-300 mt-1">
              ضريبة معتمدة: {fmtMoney(kpis.approvedVat)} ر.س
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">توزيع حالات ZATCA</CardTitle>
          </CardHeader>
          <CardContent>
            {pieData.length === 0 ? (
            ...
            {trendData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={trendData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="approved" stackId="a" fill={COLORS.approved} name="معتمدة" />
                  <Bar dataKey="rejected" stackId="a" fill={COLORS.rejected} name="مرفوضة" />
                  <Bar dataKey="pending" stackId="a" fill={COLORS.pending} name="بانتظار" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            قائمة الفواتير ({filtered.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              لا توجد فواتير ضمن المرشّحات المختارة
            </div>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">UUID / السبب</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filtered.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-xs">
                          {row.zatcaSubmittedAt ? new Date(row.zatcaSubmittedAt).toLocaleString("ar-SA") : "—"}
                        </TableCell>
