...
export default function Layout({ children }: LayoutProps) {
  ...
  const planLabel =
    user?.subscription?.plan === "starter" ? "مبتدئ" :
    user?.subscription?.plan === "professional" ? "احترافي" :
    user?.subscription?.plan === "enterprise" ? "مؤسسي" : null;
      ...
      {user?.company && (
        <div className="mx-3 mt-3 mb-1 rounded-lg border bg-sidebar-accent/40 px-3 py-2.5">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-mono text-sidebar-foreground/50 truncate">
      ...
      <div className="border-t border-sidebar-border p-3">
        ...
        {user?.subscription && (
          <div className="mb-2 flex items-center gap-2 px-2 py-1.5 rounded-md bg-sidebar-accent/30 text-xs text-sidebar-foreground/60">
            ...
            <span>
              ينتهي الاشتراك: {user.subscription.endDate
                ? new Date(user.subscription.endDate).toLocaleDateString("ar-SA")
                : "—"}
            </span>
          </div>
        )}
            ...
            <button className="flex w-full items-center gap-3 rounded-lg px-2 py-2 hover:bg-sidebar-accent transition-colors text-right">
              ...
              <div className="flex-1 min-w-0 text-right">
                ...
                <p className="text-xs text-sidebar-foreground/50 truncate">
                  {user?.role === "admin" ? "مدير" : user?.role === "superadmin" ? "مشرف عام" : "مستخدم"}
                </p>
              </div>
