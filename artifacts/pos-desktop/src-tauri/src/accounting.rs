...
/// Arabic label for a JE `source_type` (used in the posting-center block message).
fn source_type_label(t: &str) -> &str {
    match t {
        "goods_receipt" => "سند استلام",
        "goods_delivery" => "سند صرف/تسليم",
        "pos" => "نقطة بيع",
        "receipt" => "سند قبض",
        "payment" => "سند صرف",
        ...
        "sale_return" | "sales_return" => "مرتجع بيع",
...
/// مركز الترحيل — unpost the given posted entries.
///
/// Manual entries flip back to draft (their own lifecycle). Entries that were
/// AUTO-GENERATED by a source document are NOT left as stray drafts: they are
/// routed to the document's own full unpost, which reverses the GL **and** the
/// stock, DELETES the JE, and reverts the document to draft — keeping the
/// document and its JE in lockstep. Source documents without a standalone unpost
/// path (returns, goods receipts/deliveries, POS, vouchers, production, opening/
/// closing) are rejected with a message pointing to the source screen, because
/// reversing only their GL here would leave stock/shadows inconsistent.
#[tauri::command]
pub fn posting_center_unpost(ids: Vec<i64>) -> Result<i64, String> {
    use std::collections::HashSet;
    // Phase 1: snapshot metadata, then DROP the read connection so the per-doc
    // unpost commands below can each open their own write transaction without
    // contending with an open handle.
    let metas: Vec<(i64, String, Option<String>, Option<i64>)> = {
        let conn = db::open().map_err(|e| e.to_string())?;
        let mut v = Vec::new();
        for id in &ids {
            let row: Option<(String, Option<String>, Option<i64>)> = conn
                .query_row(
                    "SELECT status, source_type, source_id FROM journal_entries_local WHERE id=?1",
                    params![id],
                    |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)),
                )
                .optional()
                .map_err(|e| e.to_string())?;
            if let Some((st, stype, sid)) = row {
                v.push((*id, st, stype, sid));
            }
        }
        v
    };

    // Phase 2: PRE-VALIDATE the whole selection BEFORE mutating anything. A bulk
    // unpost must not partially apply: if any selected entry is from a source type
    // we cannot safely reverse here, reject the entire batch up front so no earlier
    // entry has already been unposted by the time we hit the blocked one.
    for (_id, status, stype, _sid) in &metas {
        if *status != "posted" {
            continue; // already unposted (or a sibling will handle it) → not a blocker
        }
    ...
    let mut done = 0i64;
    // Dedupe documents so selecting BOTH JEs of one sale (sale + sale_cogs) only
    // unposts the document once.
    let mut handled_docs: HashSet<(String, i64)> = HashSet::new();

    for (id, status, stype, sid) in metas {
        if status != "posted" {
            continue; // already unposted (possibly by a cascaded sibling leg) → skip
        }
        match stype.as_deref() {
            None | Some("manual") => {
                let mut c = db::open().map_err(|e| e.to_string())?;
            ...
            Some(kind @ ("purchase" | "sale" | "sale_cogs" | "supplier_settlement")) => {
                ...
                if !handled_docs.insert((doc_kind.to_string(), s)) {
                    continue; // sibling leg already unposted this document
                }
                match doc_kind {
            ...
            Some(other) => {
                return Err(format!(
                    "القيد ناتج عن «{}» ولا يمكن فك ترحيله من مركز الترحيل — افتح المستند المصدر لفك ترحيله.",
                    source_type_label(other)
                ));
            }
        }
