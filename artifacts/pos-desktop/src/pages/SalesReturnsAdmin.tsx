...
export default function SalesReturnsAdmin() {
  ...
  async function refresh() { setRows(await listSalesReturns()); }
  useEffect(() => {
    void refresh();
    void (async () => {
  ...
  return (
    <div style={{ padding: 12 }}>
      <Table>
        ...
        <tbody>
          {r.lines.map((l, i) => (
            <tr key={l.id ?? i}><Td>{l.itemName}</Td><Td num>{l.qty}</Td><Td num>{fmt(l.unitPrice)}</Td><Td num>{fmt(l.lineTotal)}</Td></tr>
          ))}
