...
// Inline editing inside the grid (no modal).

import { useState } from "react";
import { listUom, createUom, updateUom, deleteUom, type Uom, type CreateUomInput } from "../lib/uom";

const emptyInput: CreateUomInput = { nameAr: "", nameEn: "", shortCode: "", baseQty: 1, isDefault: false };

type EditState =
  | { mode: "new"; data: CreateUomInput }
  | { mode: "edit"; id: Uom["id"]; data: CreateUomInput }
  | null;
...
export default function UomAdmin() {
  ...
  function startEdit(u: Uom) {
    ...
    setEdit({ mode: "edit", id: u.id, data: {
      nameAr: u.nameAr, nameEn: u.nameEn ?? "", shortCode: u.shortCode ?? "",
      baseQty: u.baseQty, isDefault: u.isDefault ?? false,
    } });
  }
  function cancel() { setEdit(null); setErr(null); }
  function setField<K extends keyof CreateUomInput>(k: K, v: CreateUomInput[K]) {
  ...
  function save() {
    ...
    if (f.baseQty <= 0) { setErr("المعامل يجب أن يكون أكبر من صفر"); return; }
    if (edit.mode === "new") createUom(f); else updateUom(edit.id, f);
    ...
    <div dir="rtl" style={S.wrap}>
      ...
      <table style={S.table}>
        ...
        <tbody>
          ...
          {rows.map((u) => (
            edit?.mode === "edit" && edit.id === u.id ? (
              <EditRow key={u.id} data={edit.data} setField={setField} onSave={save} onCancel={cancel} err={err} />
            ) : (
              <tr key={u.id} style={{ ...S.tr, opacity: edit ? 0.6 : 1 }}>
                ...
                <td style={S.tdRight}>
                  <button onClick={() => startEdit(u)} disabled={!!edit} style={S.btnEdit}>تعديل</button>
                  <button onClick={() => handleDelete(u)} disabled={!!edit} style={S.btnDel}>حذف</button>
                </td>
              </tr>
            )
          ))}
...
function EditRow({ data, setField, onSave, onCancel, err, isNew }: {
  data: CreateUomInput;
  setField: <K extends keyof CreateUomInput>(k: K, v: CreateUomInput[K]) => void;
  ...
  return (
    <>
      <tr style={{ ...S.tr, background: isNew ? "#f0fdf4" : "#eff6ff" }}>
        ...
        <td style={S.td}><input value={data.shortCode ?? ""} onChange={(e) => setField("shortCode", e.target.value)} style={ci} placeholder="PCS, KG…" /></td>
        <td style={S.td}><input type="number" step="0.001" min="0.001" value={data.baseQty} onChange={(e) => setField("baseQty", Number(e.target.value))} style={ci} /></td>
        <td style={S.td}><input type="checkbox" checked={data.isDefault ?? false} onChange={(e) => setField("isDefault", e.target.checked)} /></td>
        <td style={S.tdRight}>
      ...
      {err && (
        <tr><td colSpan={6} style={{ ...S.td, background: "#fef2f2", color: "#991b1b" }}>⚠️ {err}</td></tr>
      )}
    </>
  );
}
