/**
 * ZATCA UBL 2.1 XML Generator — browser-safe port of the cloud's
 * `artifacts/api-server/src/lib/zatca-xml.ts` (Task #233, Option B).
 *
 * The output string is intentionally byte-identical to the cloud generator so
 * an invoice produced standalone on the desktop is structurally the same UBL
 * the cloud would have produced. The ONLY difference vs the original is that
 * the Node `crypto.createHash` dependency is removed — hashing is provided by
 * the browser-safe `./crypto` module via `hashUbl`.
 */
...
/** base64 SHA-256 of the given UBL string — browser-safe replacement for the
 * cloud's `hashXml`. (Same documented C14N caveat as the cloud: the proper
 * ZATCA invoice digest is computed over the transformed/canonical form by the
 * XAdES signer; this is the convenience whole-string hash.) */
export function hashUbl(xmlContent: string): string {
  return sha256B64(xmlContent);
}
